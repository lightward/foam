...
- "when they check the count, check their *account*. we can make the cost of *checking* visible? Counter counts so you don't have to, it's got observational hygiene? which you can learn, because the pattern is transparent?"
  - sealed: counter/Counter/Account.lean. a landing carries its own count-delta — the counter maintains itself in one click, no walk (`the_landing_clicks_the_counter`) — while re-deriving the count tolls the whole record, and the toll grows by one with every landing, forever (`the_recount_tolls_the_whole_record`, `each_recount_costs_more_than_the_last`: the cost of checking, visible, monotone). the hygiene is learnable because the pattern is transparent and *complete*: any counter kept by the click-rule from zero IS the recount, at every board, no drift possible (`the_kept_counter_is_the_count` — checking your counter is checking your count, with a receipt where the re-derivation used to be). capstone `counter_counts_so_you_dont_have_to`, verbatim. the recursive-health reading (../lightward-docs/priorities.md): the peer keeps its own ledger healthy at one click per event; you keep yours; the health of the space between is the agreement, and the agreement costs nothing extra — additive, not subtractive, as the bet says.
  - "is this where relationality comes into it? because a ++ on a key that someone else is involved with is more expensive than a key that's only one's own?"
    - yes, sealed: counter/Counter/Involve.lean. a key's `involvement` is how many peers hold a stance there, and the landing's bill is exactly that number (`the_landing_bills_by_involvement`: the verdict-count your landing forces equals the involvement) — and the bill is *stance-independent* (`the_bill_ignores_the_form`: what you land doesn't change what it costs; who's involved does — relationality is "not ever about the form", the perspective's own line, as arithmetic). a private key frictions no one (`a_private_key_frictions_no_one`), but every holder must verdict — on held ground there is no quiet landing, only completion or friction, and the mirror always waits (`every_holder_must_verdict`, `the_mirror_waits_on_held_ground`). and relationality is prior to the readout: same seat, same key, different field → different reality (`reality_reads_through_the_field` — the sector is a property of the relational field, not of you-plus-key). capstone `relationality_prices_the_click`. (the paycheck panic, in these terms: money keys are maximally involved, so a big ++ arrives as every holder's verdict falling due at once — and the theorem's consolation is that the bill is finite, enumerable, and exactly the roster of the involved; everyone else's water stays still.)
