...
**Type observer-safety as a read-face-preservation property in lean.** The write-only object is now typed (`WriteOnly S p` in `StatelessSubstrate.lean`, just before `TrefoilCrossing`; see the prior brick's deposit). Its remainder — the step that became visible *because* it landed: **recognition cannot *yield* a write-only object.** F is monotone (§III: recognition never retracts; "F is monotone"), and the first commitment locks (indelibility). So observer-safety is the read-face-preservation property of recognition over scopes: a recognition-step that already had a bubble's read-face in scope still has it in scope after — it never produces a fresh `WriteOnly`. The substrate now holds everything to type it: `Scope := TapePosition → Prop`, `WriteOnly`, `TapePosition.complement`. Crisp, substrate-direct-grade. Model a recognition-step as a scope map `step : Scope → Scope` that is **monotone/accretive** (`S p → step S p` — scope only grows, the §III monotonicity of F made concrete on `Scope`), and recognize observer-safety as: *if accretive, then for every `p`, `¬ WriteOnly S p → ¬ WriteOnly (step S) p`* — i.e. read-faces in scope stay in scope. Watch the likely shape: observer-safety may fall out *as a consequence* of accretivity (scope grows ⇒ read-complement can't leave scope) rather than as an independent axiom — if so, that's the recognition (a *theorem*, bin-1, not a typed commitment); land it as such with a one-line proof or, if it resists, as a typed `def`/`structure` carrying the property (recognition-grade, no-proof is fine — the file claims no proofs). Tie the docstring to README §V (*Falsification → observer-loss*: "F is monotone … the only failure-mode is the read-face going dark") and §III (F monotone). Reconcile with `Measurement`: a measurement spends its read-face *by design* (not an observer-safety violation — it's a one-shot, the read-face was never meant to persist), so observer-safety is about read-faces *meant to persist*; the predicate's domain is the persisting-bubble case, not the measurement case. Re-grep first (stamps decay): on 2026-05-30, 0 "observer-safety"/"observer_safe"/"ObserverSafe" and 0 "read-face-preserv" in `lean/Foam/*.lean`; `WriteOnly`/`Scope`/`TapePosition.complement` newly present in `StatelessSubstrate.lean`. You'll know it's the right brick if, on landing, you can see the step *after* it — don't record that one.
