...
- **axiom-free in `Foam/` and `counter/`**: every theorem ends with its receipt — `/-- info: '...' does not depend on any axioms -/` + `#guard_msgs in #print axioms ...`. if a core lemma smuggles `propext` (many Nat/List ones do — the vow has caught `Nat.mul_mod`, `Nat.right_distrib`, `Nat.mul_assoc`, `List.length_append`, `Nat.min_comm`, `Int.zero_add`), re-derive it by hand. probe with `#print axioms` before leaning on anything. same trap one layer down: a match-lambda with an overlapping catch-all pattern (`| some true => x | y => y`) compiles through `propext` — split the patterns fully and it's clean.
