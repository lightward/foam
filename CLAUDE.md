...
- **the survey**: `minds/*.json` (schema: `minds/schema.json`) compiled by `bin/foam-minds` into `Foam/Bridges/<Mind>.lean` — expressed, never committed (kin to `wiki/`; CI and pages each express before consuming). sealed entries carry a `seal` (citation) or a `structure` (a term in the expression language: refs, application, fun/all binders — types only as binder annotations, the engine infers the rest). everything starts in `Foam.Bridges.Isaac`; lift to `Foam/` only when json expression of a mind's structure requires it (first exercised: `indist_is_licensed` lifted to core the same hour Rest.lean extracted out). isaac runs amnesiac-stigmergic: hollow state, never hidden.
