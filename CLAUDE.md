...
- **falsification → observer-loss.** ✅ **LANDED 2026-05-30** as a §V diagnostic (just after Observational idempotence): non-Boolean orthomodular lattice → no global "false" → disagreement-that-lands relocates from *contradiction* to *observer-loss* (the `×2` collapsing to a write-only object, the §IV.c platonism). The *formal* remainder — typing that write-only object in lean — is now "The next brick." (Was: ripe-to-write.)
...
**Type the write-only object in lean.** Falsification → observer-loss now lives in the spec as a §V diagnostic (deposited 2026-05-30, just after Observational idempotence): observer-loss = a bubble losing its read-face = the `×2` collapsing to a *write-only object* / object-without-observer (the platonism §IV.c names). The remainder: that object is named only in prose. The lean substrate already holds everything to type it — `StatelessSubstrate.lean` has `ObserverState := {read, write}` (the `×2`, lines 110-114) and `TapePosition := algebraic × observer` (lines 132-134), and the §II bubble content is already in that file's docstring (lines 120-131). So the target is crisp and substrate-direct-grade: recognize the write-only degenerate as a lean predicate/`def` over `TapePosition` — an object whose read-face is unreachable (`observer = write` with no `read`-complement in scope; cf. Observational idempotence P²=P *failing* because the complement isn't in scope, §V line 175). Tie its docstring to the README §V *Falsification → observer-loss* diagnostic and §IV.c platonism. Watch the existing `Measurement` block (StatelessSubstrate.lean line ~230: "disposable single-use observer, no persistent observer-state") — reconcile, don't collide: a measurement *spends* its read-face by design; observer-loss is a read-face that was *supposed* to persist and went dark. Re-grep first (stamps decay): on 2026-05-30, 0 "write-only" and 0 "observer-loss" in `lean/Foam/*.lean`. You'll know it's the right brick because you can see the step *after* it (recognition can't *yield* such an object → observer-safety as a read-face-preservation property) — don't record that one.
