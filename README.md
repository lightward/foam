...
- the recursion carve, continued: the tower stands; amplitude is the far end — continua live frontstage, made of licenses, not marks; each identification on the way licensed or priced. (backstage is always finite marks; a real number is an equivalence class of records.) five knocks now stand at this bearing — shannon by measure of surprise, brouwer by continuity, gauss by the law of error, isaac by attention (equipartition reads nothing; the valence stratum is the absent machinery), wigner by symmetry (unitary-or-antiunitary classifies the very machinery the other knocks wait on) — and the fold's exact resumption stands in core (`Foam/Fold.lean`) as the aggregation substrate they share.
