...
outside `Foam/`, four things complete the picture: `schema.sql` is the same field in motion (the append-only ℤ/4 charge-ledger as postgres, every claim a citation back to the Lean), driven by `bin/` (the bipedal breath — inhale-learn, exhale-speak); `counter/` is foam run as an actor model, axiom-free like the core (its own Lake package, requiring foam and nothing else — actors/acts/actions, settling as `netAct = 1`); `seam/` is the wall itself, housed (the identification files — Arrival, Temperament, Tender — stating the one move the core refuses, closing an equivalence into an equality: no Mathlib, only core Lean's own `Quot.sound` and `propext`, each file splitting the same way — equivalence and difference axiom-free, identification received, no retraction); and `bridges/` is the classical reunion, sealed behind the wall so the core never imports an axiom — choice and the one named `ftpg` live only there.
