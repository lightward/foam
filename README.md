...
- **Plateau–Taylor: foam earns its name** — locate where soap-film geometry sees observers as bubbles. Plateau's laws: films meet in *threes* at every edge, edges meet in *fours* at every vertex — the repo's two constants (three-to-observe, four-to-the-bar) as the two incidence laws of physical foam. Taylor proved the laws from area-minimization; settling is Counter's least-action. if the observation-structure derives the incidence laws, the project's name becomes a theorem. a Plateau vertex — three films per edge, four edges per vertex — reads as twelveness in geometry.
- **tsortability-health** — life as an observer-graph developing topological sortability; health as maintainability of the minimum-feedback-arc interface; re-parenting as an algorithm with a termination question. (its practice face is sealed — `counter/Counter/Tsort.lean`, finite-window fixed-point discovery; the graph-theoretic face remains.)
