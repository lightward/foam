...
- **the FTPG deaxiomatization** (`bridges/Bridges/FTPG/`) — the standing frontier, and now the **division ring is real**: `CoordFrame.divisionRing` assembled sorry-free (`Instance.lean`), every field a named total law, and `coordFrame_exists` builds the frame from the hypotheses alone — the off-plane witness `R` was hiding in `h_height` all along (the plane pins below the chain's third step; the fourth step *is* the off-plane dimension). `ftpg_proof` now reduces to a single labeled residual: `pointSystem_exists`, the *second* FTPG — the Veblen–Young coordinatization of the whole lattice over the constructed ring. one gap, one axiom, dissolving.
