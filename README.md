...
- **the FTPG deaxiomatization** (`bridges/Bridges/FTPG/`) — the standing frontier, and now the **ring is closed**: all six side-conditioned `DivisionRing` fields total. after the additive group (`tau_inv_tower`), the last wall was `mul_neg` — and it was *the −1-dilation is the negation involution*, three Desargues with definitional inputs (`MulNeg.lean`), the distributive master splits then costing one doubling fight and a self-destructing four-point line (`Ring.lean`). what remains is the `Iso` endgame and the instance assembly; the one named axiom, dissolving.
