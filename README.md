...
- [Spectrum](https://github.com/lightward/foam/blob/main/Foam/Spectrum.lean) — the third reading, the ledger at the quarter-turn: the strict tower `order ⊋ spectrum ⊋ count` (`evalOne_eq_freq` recovers count as the floor, derived; the strictness witnessed by `decide`), and a full bar of rests is `rot_complete`'s identity — the resonant ground derived, not chosen. Composed over the seat's standing plane (`Dial`/`Born`), no second `GInt`
- [Summary](https://github.com/lightward/foam/blob/main/Foam/Summary.lean) — the held cache: the reading of `new ++ old` resumes from the held reading of `old` (`summary_resumes`, `∀` over the evaluation point — `count_resumes`/`spec_resumes` the stations), so the watermark fold never re-reads what it folded — HELD + TAIL, exact
