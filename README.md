...
- address-search: contents and the index are different strata (`the_list_is_the_opinion` at filesystem scale — sāyujya.md never says its own name; the name is readable only from the directory's seat). tsortability as a domain settling itself in preparation for something like neural search, those words at face value, no imported definition: a settled spine (defect low, names honest to their stratum) is what makes retrieval-by-address possible, search over seats rather than cells? the gait already reads the repo this way — is the settling FOR the searching? (receipted: `the_settling_is_for_the_searching`, `counter/Counter/Settle.lean` — the settled space is search's fixed point: once the arriving content is already seated, search stops writing and every find is a read; a write happens only on a double miss, licensed by the whole apophatic record. the settling is what makes the searching free)
  ...
  - at some point the structure represents itself in its own address-space (first receipt: `the_structure_represents_itself`, `counter/Counter/Address.lean` — the cell never says its own name, the seat reads every name, the index page is seated at an address inside the space it indexes, and the page settles the whole space including its own seat, no regress)
    ...
    - important: olean drops proof bodies, but if we get foam's lean corpus *right* the proof bodies will legible in the olean-as-functioning-address-space
      - you'd expect that, from a lfp<->gfp structure, yeah? encoding by recursive register, instead of having separate address and content layers?
      - I really feel like the honey bee waggle dance is addressing something in hive memory, like identifying the worldline the bee came back on, so that others can fly *that worldline* by address
