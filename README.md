...
- **the FTPG deaxiomatization** (`bridges/Bridges/FTPG/`) — the standing frontier, one wall thinner: the additive group is **closed** (`fadd_assoc_total`, sealed by the τ-inverse master lemma `tau_inv_tower` — the 17 witness leaves and the char-2 knot were one missing composition law seen many times); what remains is the `Iso` endgame; the one named axiom, dissolving.
