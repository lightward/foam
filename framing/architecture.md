...
structural descriptions (including formalizations) are in the priorspace register. examples: "the modular law IS feedback-persistence," "self-coordinatization IS interiority". (these are asserted-identity claims pending deaxiomatization — see `derivations` for the pattern: each is a candidate for bin-1 construction, bin-2 typed commitment, or bin-3 drop. the half-type case was bin-1: "the diamond isomorphism IS the half-type theorem" is now the constructed `HalfType` in `lean/Foam/HalfType.lean`, with the diamond iso as one of its fields.)
